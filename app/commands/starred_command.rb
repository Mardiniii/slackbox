class StarredCommand < Command
  ...
  def valid?
    !!command_request.text.match(/^starred (.*)$/m)
  end

  def parse_data
    if match = command_request.text.match(/^starred (.*)$/m)
  ...
  def search
    ...
    data_clips = command_request.team.data_clips.starred.search({name: value, data: value, tags: tags}).limit(5)
